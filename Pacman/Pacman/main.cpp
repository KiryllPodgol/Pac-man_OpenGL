...
public:
	...
	bool loadMTL(const std::string& filename) {
		...
		while (std::getline(file, line)) {
			...
			if (type == "newmtl") {
			...
			else if (type == "illum") { // Illumination model (игнорируем)
			...
			else if (type == "Pr" || type == "Pds" || type == "Pl") { // Нестандартные параметры Blender

				// Пропускаем нестандартные параметры

			}

			else if (type == "Ke") { // Emissive (игнорируем)
				...
				std::cout << "Unknown MTL parameter: " << type << std::endl;
		...
		std::cout << "Loaded " << materialsCount << " materials from " << filename << std::endl;
	...
	void renderWithMaterials() const {
		...
		for (size_t i = 0; i < indices.size(); i++) {
			...
			// Применяем материал если он изменился

			if (i < materialNames.size() && materialNames[i] != currentMaterial) {
	...
	void applyMaterial(const std::string& materialName) const {
		...
		if (it != materials.end()) {
			...
			std::cout << "Applied material: " << materialName << " with color ("
			...
			glColor3f(1.0f, 1.0f, 0.0f); // Желтый по умолчанию для Пакмана

			std::cout << "Using default material for: " << materialName << std::endl;
...
void drawPacman3D(float x, float y, float z, float size, float mouthAngle, float rotationY) {
	...
	// Включаем освещение для модели с материалами

	glEnable(GL_LIGHTING);



	// Убираем принудительный цвет - используем материалы из OBJ/MTL
...
int main(int argc, char** argv) {
	...
	if (pacmanModelLoaded) {
		...
		pacmanModel.scale(1.0f); // Настройте масштаб под вашу модель
	...
	std::cout << "Press 'C' to toggle camera view" << std::endl;
